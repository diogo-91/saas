                                    ...
                                    <Draggable key={stage.id} draggableId={stage.id.toString()} index={index}>
                                        {(provided) => (
                                            <div ref={provided.innerRef} {...provided.draggableProps} className="flex flex-col w-80 bg-muted/50 dark:bg-muted/20 rounded-xl border h-full max-h-full">
                                                <div {...provided.dragHandleProps} className="p-3 border-b bg-slate-900 border-slate-800 rounded-t-xl flex justify-between items-center sticky top-0 z-10 group cursor-grab active:cursor-grabbing">
                                                    <div className="flex items-center gap-2">
                                                        <span className="text-base">{stage.emoji}</span>
                                                        <h3 className="font-semibold text-slate-50 text-sm uppercase">{stage.name}</h3>
                                                    </div>
                                                    <div className="flex items-center gap-2">
                                                        <Badge variant="secondary" className="bg-slate-800 text-slate-50 hover:bg-slate-700">{columns[stage.id]?.length || 0}</Badge>
                                                        <DropdownMenu>
                                                            <DropdownMenuTrigger asChild><Button variant="ghost" size="icon" className="h-6 w-6 text-slate-400 hover:text-slate-50 hover:bg-slate-800"><MoreHorizontal className="h-3 w-3" /></Button></DropdownMenuTrigger>
                                                            <DropdownMenuContent>
